...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	userRepository := repository.ProvideUserRepository(client, db, concurrencyService)
	redeemCodeRepository := repository.NewRedeemCodeRepository(client)
	refreshTokenCache := repository.NewRefreshTokenCache(redisClient)
	settingRepository := repository.NewSettingRepository(client)
	groupRepository := repository.NewGroupRepository(client, db)
	proxyRepository := repository.NewProxyRepository(client, db, schedulerCache)
	proxyPoolRepository := repository.NewProxyPoolRepository(client, db)
	proxyPoolService := service.NewProxyPoolService(proxyPoolRepository)
	...
	currencyCutoverCacheBypass := repository.ProvideCurrencyCutoverCacheBypass(configConfig)
	billingCache := repository.ProvideCurrencyAwareBillingCache(redisClient, currencyCutoverCacheBypass)
	...
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, currencyCutoverCacheBypass, billingCacheService, concurrencyService)
	...
	subscriptionService := service.ProvideSubscriptionService(groupRepository, userSubscriptionRepository, billingCacheService, client, configConfig, subscriptionResetCardRepository)
	...
	schedulerSnapshotService := service.ProvideSchedulerSnapshotService(schedulerCache, schedulerOutboxRepository, accountRepository, groupRepository, configConfig, leaderLockCache, db)
	...
	subscriptionCacheInvalidationOutboxRepository := repository.NewSubscriptionCacheInvalidationOutboxRepository(db)
	subscriptionCacheInvalidationWorker := service.ProvideSubscriptionCacheInvalidationWorker(subscriptionCacheInvalidationOutboxRepository, subscriptionService)
	opsService := service.ProvideOpsService(opsRepository, settingRepository, configConfig, accountRepository, userRepository, concurrencyService, gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, opsSystemLogSink, settingService, authCacheInvalidationWorker, subscriptionCacheInvalidationWorker, apiKeyService)
	...
	codexTurnStatePanelHandler := admin.NewCodexTurnStatePanelHandler()
	...
	attachmentGatewayHandler := admin.NewAttachmentGatewayHandler(attachmentR2Service)
	...
	grokOAuthHandler := admin.NewGrokOAuthHandlerWithProxyPool(grokOAuthService, adminService, grokQuotaService, tokenRefreshService, proxyPoolService)
	...
	gateway, err := service.ProvideUnifiedPaymentGateway(configConfig, settingService)
	if err != nil {
		return nil, err
	}
	unifiedWebhookInboxStore := repository.NewUnifiedPaymentWebhookInboxStore(db)
	paymentService, err := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, apiKeyAuthCacheInvalidator, billingCacheService, concurrencyService, gateway, unifiedWebhookInboxStore)
	if err != nil {
		return nil, err
	}
	...
	usageCleanupService := service.ProvideUsageCleanupService(usageCleanupRepository, timingWheelService, dashboardAggregationService, configConfig, leaderLockCache, db)
	...
	accountPoolRepository := repository.NewAccountPoolRepository(client, db)
	accountPoolService := service.NewAccountPoolService(accountPoolRepository, usageLogRepository, configConfig)
	accountPoolHandler := admin.NewAccountPoolHandler(accountPoolService)
	proxyPoolHandler := admin.NewProxyPoolHandler(proxyPoolService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	openCodeGoUsageService := service.ProvideOpenCodeGoUsageService(accountRepository, httpUpstream, settingService, leaderLockCache, db)
	bpsProbeService := service.NewBPSProbeService(db, accountRepository, accountTestService)
	bpsUpstreamHandler := admin.NewBPSUpstreamHandler(settingService, adminService, bpsProbeService)
	adminHandlers := handler.ProvideAdminHandlers(codexTurnStatePanelHandler, bpsUpstreamHandler, dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, attachmentGatewayHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, accountPoolHandler, proxyPoolHandler, upstreamBillingProbeService, ollamaCloudUsageService, openCodeGoUsageService)
	...
	gatewayHandler := handler.ProvideGatewayHandler(gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, subscriptionService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, coordinator)
	openAIAbnormalRetryRegistrar := repository.NewOpenAIAbnormalRetryRegistrar(redisClient)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, pluginManager, concurrencyService, billingCacheService, subscriptionService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, openAIAbnormalRetryRegistrar, attachmentR2Service, configConfig, coordinator)
	...
	paymentRefundReconciliationStore := repository.NewPaymentRefundReconciliationStore(db)
	paymentRefundReconciliationService := service.ProvidePaymentRefundReconciliationService(paymentService, paymentRefundReconciliationStore)
	healthService := server.ProvideHealthService(db, redisClient, paymentRefundReconciliationService)
