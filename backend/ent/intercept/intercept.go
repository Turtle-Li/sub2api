...
// The AccountPoolFunc type is an adapter to allow the use of ordinary function as a Querier.
type AccountPoolFunc func(context.Context, *ent.AccountPoolQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f AccountPoolFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.AccountPoolQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.AccountPoolQuery", q)
}

// The TraverseAccountPool type is an adapter to allow the use of ordinary function as Traverser.
type TraverseAccountPool func(context.Context, *ent.AccountPoolQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseAccountPool) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseAccountPool) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.AccountPoolQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.AccountPoolQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.AccountPoolQuery:
		return &query[*ent.AccountPoolQuery, predicate.AccountPool, accountpool.OrderOption]{typ: ent.TypeAccountPool, tq: q}, nil
