...
func (_q *PaymentOrderQuery) QueryInvoiceRequest() *PaymentInvoiceRequestQuery {
...
func (_q *PaymentOrderQuery) WithInvoiceRequest(opts ...func(*PaymentInvoiceRequestQuery)) *PaymentOrderQuery {
	query := (&PaymentInvoiceRequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	...
	if query := _q.withInvoiceRequest; query != nil {
		if err := _q.loadInvoiceRequest(ctx, query, nodes, nil,
			func(n *PaymentOrder, e *PaymentInvoiceRequest) { n.Edges.InvoiceRequest = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PaymentOrderQuery) loadInvoiceRequest(ctx context.Context, query *PaymentInvoiceRequestQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *PaymentInvoiceRequest)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(paymentinvoicerequest.FieldOrderID)
	}
	query.Where(predicate.PaymentInvoiceRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentorder.InvoiceRequestColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
