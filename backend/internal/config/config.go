...
const DefaultCSPPolicy = "default-src 'self'; worker-src 'self' blob:; script-src 'self' __CSP_NONCE__ https://challenges.cloudflare.com https://*.alicdn.com https://static.cloudflareinsights.com https://turing.captcha.qcloud.com https://turing.captcha.gtimg.com https://ca.turing.captcha.qcloud.com https://global.turing.captcha.gtimg.com https://www.tycaptcha.com https://cloudcache.tencentcs.com https://*.stripe.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; style-src 'self' 'unsafe-inline' https://*.captcha.gtimg.com https://fonts.googleapis.com https://*.alicdn.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; img-src 'self' data: blob: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https://turing.captcha.qcloud.com https://www.tycaptcha.com https://rce.tencentrio.com https:; frame-src 'self' https://challenges.cloudflare.com https://turing.captcha.qcloud.com https://ca.turing.captcha.qcloud.com https://www.tycaptcha.com https://*.stripe.com https://checkout.airwallex.com https://checkout-demo.airwallex.com https://openapi.alipay.com https://openapi-sandbox.dl.alipaydev.com https://excashier.alipay.com https://unitradeprod.alipay.com https://*.alipay.com https://*.alipaydev.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
...
type ProxyProbeConfig struct {
	...
	// 留空时使用内置默认列表（ipwho → ip-api → chatgpt-trace → ipify）。
...
type ProbeURLConfig struct {
	...
	Parser string `mapstructure:"parser"` // "ipwho" / "ip-api" / "ipify" / "chatgpt-trace"
}
...
func normalizeProxyProbeURLs(targets []ProbeURLConfig) ([]ProbeURLConfig, error) {
	...
	for i, target := range targets {
		...
		switch parser {
		case "ipwho", "ip-api", "ipify", "chatgpt-trace":
		default:
...
// applyReleaseConcurrencyDefaults enables bounded image backpressure for the
// production image even when the host's persisted config predates these keys.
// Every value remains explicitly overridable through config.yaml or the
// corresponding environment variable; debug/dev defaults stay conservative.
func applyReleaseConcurrencyDefaults(cfg *Config) {
	if cfg == nil || cfg.Server.Mode != "release" {
		return
	}
	if !configOrEnvExplicit("batch_image.worker_concurrency", "BATCH_IMAGE_WORKER_CONCURRENCY") {
		cfg.BatchImage.WorkerConcurrency = 2
	}
	if !configOrEnvExplicit("gateway.image_concurrency.enabled", "GATEWAY_IMAGE_CONCURRENCY_ENABLED") {
		cfg.Gateway.ImageConcurrency.Enabled = true
	}
	if !configOrEnvExplicit("gateway.image_concurrency.max_concurrent_requests", "GATEWAY_IMAGE_CONCURRENCY_MAX_CONCURRENT_REQUESTS") {
		cfg.Gateway.ImageConcurrency.MaxConcurrentRequests = 16
	}
	if !configOrEnvExplicit("gateway.image_concurrency.overflow_mode", "GATEWAY_IMAGE_CONCURRENCY_OVERFLOW_MODE") {
		cfg.Gateway.ImageConcurrency.OverflowMode = ImageConcurrencyOverflowModeWait
	}
}

func configOrEnvExplicit(configKey, envKey string) bool {
	if viper.InConfig(configKey) {
		return true
	}
	_, ok := os.LookupEnv(envKey)
