...
func (h *PaymentHandler) ListOrders(c *gin.Context) {
	...
	orders, total, err := h.paymentService.AdminListOrders(c.Request.Context(), userID, service.OrderListParams{
		...
		InvoiceEmailStatus: c.Query("invoice_email_status"),
	...
	presentations, err := h.paymentService.InvoiceOrderPresentations(c.Request.Context(), orders)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	response.Paginated(c, sanitizeAdminPaymentOrdersForResponse(orders, presentations), int64(total), page, pageSize)
}
...
func (h *PaymentHandler) GetOrderDetail(c *gin.Context) {
	...
	presentation, err := h.paymentOrderInvoicePresentation(c.Request.Context(), order)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	auditLogs, _ := h.paymentService.GetOrderAuditLogs(c.Request.Context(), orderID)
	response.Success(c, gin.H{"order": sanitizeAdminPaymentOrderForResponse(order, presentation), "auditLogs": auditLogs})
}

// UpdateInvoiceRequest advances a manual invoice through processing, issuing,
// or rejection.
// PUT /api/v1/admin/payment/orders/:id/invoice
func (h *PaymentHandler) UpdateInvoiceRequest(c *gin.Context) {
	orderID, ok := parseIDParam(c, "id")
	if !ok {
		return
	}
	var req service.AdminUpdateInvoiceInput
	if strings.HasPrefix(strings.ToLower(c.GetHeader("Content-Type")), "multipart/form-data") {
		c.Request.Body = http.MaxBytesReader(c.Writer, c.Request.Body, service.MaxInvoicePDFBytes+(1<<20))
		defer func() {
		...
		file, header, err := c.Request.FormFile("invoice_pdf")
		if err == nil {
			defer func() { _ = file.Close() }()
			data, readErr := io.ReadAll(io.LimitReader(file, service.MaxInvoicePDFBytes+1))
			if readErr != nil {
				response.BadRequest(c, "Unable to read invoice PDF")
				return
			}
			req.Document = &service.InvoicePDFInput{Filename: header.Filename, Data: data}
		} else if !errors.Is(err, http.ErrMissingFile) {
			response.BadRequest(c, "Invalid invoice PDF upload")
			return
		}
	...
	invoice, err := h.paymentService.AdminUpdateOrderInvoiceRequest(c.Request.Context(), orderID, getAdminIDFromContext(c), req)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// RetryInvoiceEmail immediately re-enqueues a failed, pending, or stale claim
// and returns the direct invoice DTO expected by the admin client.
// POST /api/v1/admin/payment/orders/:id/invoice/email/retry
func (h *PaymentHandler) RetryInvoiceEmail(c *gin.Context) {
	orderID, ok := parseIDParam(c, "id")
	if !ok {
		return
	}
	invoice, err := h.paymentService.AdminRetryInvoiceEmail(c.Request.Context(), orderID, getAdminIDFromContext(c))
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// RetryInvoiceFeishuNotification re-enqueues the durable, privacy-minimal
// request notification. It returns the direct invoice DTO.
// POST /api/v1/admin/payment/orders/:id/invoice/feishu/retry
func (h *PaymentHandler) RetryInvoiceFeishuNotification(c *gin.Context) {
	orderID, ok := parseIDParam(c, "id")
	if !ok {
		return
	}
	invoice, err := h.paymentService.AdminRetryInvoiceFeishuNotification(c.Request.Context(), orderID, getAdminIDFromContext(c))
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
type AdminPaymentOrderResult struct {
	...
	RefundRecovery          *service.PaymentRefundRecoveryPresentation `json:"refund_recovery,omitempty"`
...
func sanitizeAdminPaymentOrdersForResponse(orders []*dbent.PaymentOrder, presentations map[int64]service.PaymentOrderInvoicePresentation) []*AdminPaymentOrderResult {
	...
	for _, order := range orders {
		if item := sanitizeAdminPaymentOrderForResponse(order, presentations[order.ID]); item != nil {
			out = append(out, item)
		}
	}
	return out
...
func sanitizeAdminPaymentOrderForResponse(order *dbent.PaymentOrder, projected ...service.PaymentOrderInvoicePresentation) *AdminPaymentOrderResult {
	if order == nil {
		return nil
	}
	presentation := defaultAdminPaymentOrderInvoicePresentation(order)
	if len(projected) > 0 {
	...
	return &AdminPaymentOrderResult{
		...
		RefundRecovery:          presentation.RefundRecovery,
		InvoiceEligible:         presentation.InvoiceEligible,
		Invoice:                 presentation.Invoice,
	}
}

func (h *PaymentHandler) paymentOrderInvoicePresentation(ctx context.Context, order *dbent.PaymentOrder) (service.PaymentOrderInvoicePresentation, error) {
	presentations, err := h.paymentService.InvoiceOrderPresentations(ctx, []*dbent.PaymentOrder{order})
	if err != nil {
		return service.PaymentOrderInvoicePresentation{}, err
	}
	return presentations[order.ID], nil
}
...
// AdminExternalRefundConfirmationRequest records evidence for money returned
// outside the provider API. Refund identity and amount remain server-owned.
type AdminExternalRefundConfirmationRequest struct {
	MethodCode        string    `json:"method_code" binding:"required"`
	ExternalReference string    `json:"external_reference" binding:"required"`
	RefundedAt        time.Time `json:"refunded_at" binding:"required"`
	EvidenceDetail    string    `json:"evidence_detail" binding:"required"`
}
...
// ResumeUnifiedRefund releases a proven WeChat merchant-balance pause while
// retaining the original central refund request and provider refund number.
// POST /api/v1/admin/payment/orders/:id/refund/retry
func (h *PaymentHandler) ResumeUnifiedRefund(c *gin.Context) {
	if !middleware.EnforceStepUpAlways(c, h.totpService, h.userService) {
		return
	}
	orderID, ok := parseIDParam(c, "id")
	...
	result, err := h.paymentService.ResumeUnifiedRefund(c.Request.Context(), orderID, getAdminIDFromContext(c))
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// ConfirmExternalUnifiedRefund confirms that the exact pending refund was
// completed outside the provider API and then reclaims its reserved benefit.
// POST /api/v1/admin/payment/orders/:id/refund/confirm-external
func (h *PaymentHandler) ConfirmExternalUnifiedRefund(c *gin.Context) {
	if !middleware.EnforceStepUpAlways(c, h.totpService, h.userService) {
		return
	}
	orderID, ok := parseIDParam(c, "id")
	...
	var req AdminExternalRefundConfirmationRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}
	result, err := h.paymentService.ConfirmExternalUnifiedRefund(c.Request.Context(), orderID, getAdminIDFromContext(c), service.ExternalRefundConfirmationInput{
		MethodCode: req.MethodCode, ExternalReference: req.ExternalReference,
		RefundedAt: req.RefundedAt, EvidenceDetail: req.EvidenceDetail,
	})
	if err != nil {
		response.ErrorFrom(c, err)
		return
...
func adminSubscriptionPlansForResponse(plans []*dbent.SubscriptionPlan, groupInfo map[int64]service.PlanGroupInfo) []AdminSubscriptionPlanResult {
	...
	for _, p := range plans {
		...
		result = append(result, AdminSubscriptionPlanResult{
			...
			Entitlements:    service.PlanEntitlementsFromRaw(p.Entitlements),
