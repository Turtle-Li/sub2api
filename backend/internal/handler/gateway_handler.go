...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if platform == service.PlatformComposite {
		...
		if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
			...
			availableModels = apiKey.Group.ModelAllowlist.FilterForListing(source)
...
func (h *GatewayHandler) writeVerifiedProbeModelsIfRequested(c *gin.Context, platform string, groupID *int64, modelIDs []string, custom bool) bool {
	...
	if custom {
		writeAllowlistedModelsListWithProbeMetadata(c, platform, modelIDs, metadata)
	} else {
...
func writeAllowlistedModelsList(c *gin.Context, platform string, modelIDs []string) {
	writeAllowlistedModelsListWithProbeMetadata(c, platform, modelIDs, nil)
}

func writeAllowlistedModelsListWithProbeMetadata(c *gin.Context, platform string, modelIDs []string, metadata map[string]service.ProbeModelMetadata) {
	if platform == service.PlatformOpenAI {
		writeOpenAIModelsListWithProbeMetadata(c, modelIDs, metadata)
		return
	}
...
func writeOpenAIModelsListWithProbeMetadata(c *gin.Context, modelIDs []string, metadata map[string]service.ProbeModelMetadata) {
	...
	if metadata != nil {
		...
		writeModelsListResponse(c, items)
		return
	}
	writeModelsListResponse(c, models)
}
