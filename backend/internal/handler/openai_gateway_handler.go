...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	var lastFailoverErr *service.UpstreamFailoverError
	var oauth429FailoverState service.OpenAIOAuth429FailoverState
	// Keep the attachment-prepared body as the replay baseline. A failover on
	// the first turn must not silently drop transformed file/image inputs.
	wsAttemptMessage := append([]byte(nil), forwardFirstMessage...)
