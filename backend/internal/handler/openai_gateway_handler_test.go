...
// expectedWSCloseReason 返回首帧/次轮被拒时期待的 1008 关闭原因，默认是分组模型
// 白名单文案；简易模式 key 额度窗口等其它拒绝路径通过 closeReason 覆盖。
func expectedWSCloseReason(tc openAIResponsesWSUsageLogCase) string {
	if reason := strings.TrimSpace(tc.closeReason); reason != "" {
		return reason
	}
