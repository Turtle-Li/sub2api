...
func logOpsRecoveredUpstream(c *gin.Context, ops *service.OpsService, finalStatus int) {
	...
	recordedCode := ""
	recordedMessage := "Recovered upstream error"
	...
	entry.StatusCode = recordedStatus
	entry.ErrorPhase = "upstream"
	entry.ErrorType = recordedType
	entry.ErrorSource = "upstream_http"
	entry.ErrorOwner = "provider"
	severityStatus := recordedStatus
	if severityStatus < http.StatusBadRequest && lastStatus > 0 {
		severityStatus = lastStatus
	}
	entry.Severity = classifyOpsSeverity(entry.ErrorType, severityStatus)
	entry.IsCountTokens = isCountTokensRequest(c)
	entry.CreatedAt = time.Now()
	entry.ErrorMessage = recordedMessage
