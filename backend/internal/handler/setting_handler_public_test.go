...
func TestSettingHandler_GetPublicSettings_ExposesPricingCurrency(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	require.Equal(t, service.PricingSettlementCurrencyCNY, resp.Data.PricingCurrency.SettlementCurrency)
	require.Equal(t, 6.75, resp.Data.PricingCurrency.USDToCNYRate)
}
