	...
	for _, key := range []string{"codex_credits_snapshot", "codex_referral_snapshot"} {
		for _, tc := range []struct {
			...
			t.Run(key+"/"+tc.name, func(t *testing.T) {
				...
				// The fork always wraps UpdateExtra in a transaction so the
				// post-commit sticky-session snapshot refresh cannot expose
				// uncommitted state (accountMutationClient). The invariant this
				// test owns is the absent account_changed event, not the absent
				// transaction.
				mock.ExpectBegin()
				...
				mock.ExpectCommit()
				// Snapshot-only changes must perform just the UPDATE, without an
				// account_changed event that would rebuild scheduler buckets.
