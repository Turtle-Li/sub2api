...
func TestBulkUpdateOpenCodeGoIdentityCleanupIsValueConditional(t *testing.T) {
	...
	// OpenCode 分支必须出现在 Ollama 分支之前，且 eligible 判定按统一谓词同时覆盖
	// opencode_go 平台（Go 订阅）与挂载白名单平台 + opencode 用量 URL。
	require.Contains(t, query, "type IN ('apikey', 'upstream')")
...
// F1 回归：OpenCode eligible 判定必须包含旧行 OpenCode 用量身份，否则挂载平台的
// Ollama 行在代理变化时会先命中 OpenCode 分支而遮蔽 Ollama 快照清理。这里断言
// OpenCode 分支的 WHEN 携带 opencode 正则（与 Ollama 正则互斥），真实行为由
// integration 测试覆盖。
func TestBulkUpdateOpenCodeGoEligiblePredicateIncludesBaseURL(t *testing.T) {
	...
	// 覆盖 opencode_go 平台与挂载白名单的 opencode 用量 URL，使 Ollama 行无法命中。
...
// 的成员关系都要与 IsOpenCodeGoUsageAccount（apikey/upstream + OpenCode 用量 URL）
// 一致，防止两侧平台列表各自漂移。opencode_go 走平台 + Go 订阅分支而非挂载白名单：
