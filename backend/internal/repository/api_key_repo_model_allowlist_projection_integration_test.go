...
func TestGroupModelAllowlistSharesLegacyStorageForRollingUpgrade(t *testing.T) {
	ctx := context.Background()
	group := mustCreateGroup(t, integrationEntClient, &service.Group{
		Name:     fmt.Sprintf("allowlist-rolling-%d", time.Now().UnixNano()),
		Platform: service.PlatformOpenAI, RateMultiplier: 1,
		ModelAllowlist: service.GroupModelAllowlist{Enabled: false, Models: []string{"gpt-5.5"}},
	})
	t.Cleanup(func() {
		_, err := integrationDB.ExecContext(ctx, "DELETE FROM groups WHERE id=$1", group.ID)
		require.NoError(t, err)
	...
	require.NoError(t, integrationDB.QueryRowContext(ctx,
		"SELECT models_list_config::text FROM groups WHERE id=$1", group.ID).Scan(&legacy))
	require.JSONEq(t, `{"enabled":false,"models":["gpt-5.5"]}`, legacy)
	_, err := integrationDB.ExecContext(ctx,
		`UPDATE groups SET models_list_config='{"enabled":true,"models":["gpt-6-*"]}' WHERE id=$1`, group.ID)
	require.NoError(t, err)
	got, err := integrationEntClient.Group.Get(ctx, group.ID)
	require.NoError(t, err)
	require.True(t, got.ModelAllowlist.Enabled)
	require.Equal(t, []string{"gpt-6-*"}, got.ModelAllowlist.Models)
	_, err = integrationEntClient.Group.UpdateOneID(group.ID).
		SetModelAllowlist(service.DomainGroupModelAllowlist(service.GroupModelAllowlist{Enabled: false, Models: []string{"gpt-5.6-sol"}})).Save(ctx)
	require.NoError(t, err)
	require.NoError(t, integrationDB.QueryRowContext(ctx,
		"SELECT models_list_config::text FROM groups WHERE id=$1", group.ID).Scan(&legacy))
	require.JSONEq(t, `{"enabled":false,"models":["gpt-5.6-sol"]}`, legacy)
}
