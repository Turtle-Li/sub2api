...
func TestAuthCacheInvalidationTriggers_CoverSecurityMutationsOnly(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 1, count(), "balance update must enqueue because balance is embedded in the auth snapshot")
	clear()

	_, err = integrationDB.ExecContext(ctx, "UPDATE users SET concurrency = concurrency + 1 WHERE id = $1", user.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "concurrency update must enqueue all active keys")
	clear()
