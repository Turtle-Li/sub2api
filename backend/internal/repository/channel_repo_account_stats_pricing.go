...
func (r *channelRepository) batchLoadAccountStatsModelPricing(ctx context.Context, ruleIDs []int64) (map[int64][]service.ChannelModelPricing, error) {
	...
	for rows.Next() {
		...
		if err := p.NormalizeCurrency(); err != nil {
			return nil, fmt.Errorf("normalize account stats model pricing currency: %w", err)
		}
...
func createAccountStatsModelPricingTx(ctx context.Context, tx *sql.Tx, ruleID int64, pricing *service.ChannelModelPricing) error {
	...
	if err != nil {
		return fmt.Errorf("normalize account stats model pricing currency: %w", err)
	}
	...
	err = tx.QueryRowContext(ctx,
		`INSERT INTO channel_account_stats_model_pricing (rule_id, platform, models, billing_mode, currency, input_price, output_price, cache_write_price, cache_write_1h_price, cache_read_price, reasoning_effort_multipliers, image_output_price, per_request_price)
