...
func TestSubscriptionResetCardPurchaseQuoteAndDurableReplay(t *testing.T) {
	...
	require.Equal(t, 15, quote.ValidityDays)
	require.False(t, quote.ExpiresAt.Before(beforeQuote.Add(15*24*time.Hour)))
	require.False(t, quote.ExpiresAt.After(afterQuote.Add(15*24*time.Hour)))
