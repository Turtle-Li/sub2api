...
func TestTrafficAdmissionDrainsWithoutInterruptingAdmittedRequest(t *testing.T) {
	...
	r.GET("/internal/refund-rollback-readiness", func(c *gin.Context) { c.Status(http.StatusOK) })
	r.POST("/internal/reviewed-refunds-rollout", func(c *gin.Context) { c.Status(http.StatusOK) })
	...
	rollbackReady := httptest.NewRecorder()
	r.ServeHTTP(rollbackReady, httptest.NewRequest("GET", "/internal/refund-rollback-readiness", nil))
	require.Equal(t, http.StatusOK, rollbackReady.Code)
	rollout := httptest.NewRecorder()
	r.ServeHTTP(rollout, httptest.NewRequest("POST", "/internal/reviewed-refunds-rollout", nil))
	require.Equal(t, http.StatusOK, rollout.Code)
	require.EqualValues(t, 1, s.InFlightRequests())
	close(finish)
	<-done
