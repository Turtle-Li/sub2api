...
func TestFunctionCodeTransportRequiresExplicitCatalogContract(t *testing.T) {
	...
	_, bridge := mustPrepare(t, source, "scope", nil)
	// functions. 宿主前缀与其他还原路径一致地被接受（见 TestFunctionCodeMarkerVariants）。
	for _, name := range []string{"", "missing", "custom_code", "plain", "numeric", "run_code ", " run_code", "functions.plain", "run_code\n", "run_code/extra"} {
