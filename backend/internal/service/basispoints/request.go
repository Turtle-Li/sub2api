...
func Prepare(raw []byte, scope string, replay *ReplayCache) ([]byte, *Bridge, error) {
	...
	if err := requirePlainTextFormat(source["text"]); err != nil {
		return nil, nil, err
	}
	...
	if len(catalog) > 0 {
		...
		protocol = "This request comes from an external Responses client. Use only the client tools in the catalog below. " +
			...
			"FUNCTION: code must contain one serialized JSON object {\"name\":\"CATALOG_NAME\",\"arguments\":{...}}. Arguments is an object, not an extra JSON string. FUNCTION code is JSON data only, never JavaScript, a shell command or prose; raw code or commands travel only through a tool's FUNCTION_CODE or CUSTOM transport. " +
			"FUNCTION_CODE: when a catalog function explicitly specifies this transport, set summary to exactly codex2api.function_code/CATALOG_NAME, put the exact argument named in its catalog entry (code or cmd) directly in native code, and serialize all other arguments as one JSON object in extended_summary ({} if none). Never repeat that argument in extended_summary. This replaces the FUNCTION envelope for that tool, so do not JSON-wrap, fence or re-escape the code or command text. " +
...
// requirePlainTextFormat rejects structured output. Sub2API keeps json_object and
// json_schema requests on the native channel instead of validating them locally.
func requirePlainTextFormat(raw any) error {
	if raw == nil {
		return nil
	}
	config, ok := raw.(object)
	if !ok {
		return fmt.Errorf("basispoints text must be an object")
	}
	if config["format"] == nil {
		return nil
	}
	format, ok := config["format"].(object)
	if !ok || text(format["type"]) != "text" {
		return fmt.Errorf("basispoints supports text output format only")
	}
	return nil
}
