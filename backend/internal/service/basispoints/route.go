...
func NativeFallbackReason(body []byte, keepLiveSearch bool) string {
	...
	// Inline data images are uploaded by Sub2API's R2 externalizer before Prepare;
	// any that remain make Prepare fail, which keeps the request on the native channel.
	return ""
}
