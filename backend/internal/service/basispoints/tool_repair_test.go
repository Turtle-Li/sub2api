...
func TestToolRepairKeepsOneResponseAndAtomicToolBatch(t *testing.T) {
	...
	// 只回报最后一次续轮的用量，上下文规模不被累加。
	require.Equal(t, json.Number("20"), usage["input_tokens"])
	require.Equal(t, json.Number("4"), usage["output_tokens"])
	require.Equal(t, json.Number("24"), usage["total_tokens"])
	require.Equal(t, json.Number("2"), repairValue[object](t, usage["input_tokens_details"])["cached_tokens"])
...
func TestToolRepairBoundsAttemptsAndReportsLatestUsage(t *testing.T) {
...
func TestToolRepairDoesNotRetryOtherFailures(t *testing.T) {
	for _, name := range []string{"unsupported_native", "undeclared_target", "missing_call_id", "invalid_outer_json", "incomplete", "failed", "missing_terminal_tool", "truncated"} {
...
func TestToolRepairRestoresOriginalRawPayload(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.functionCode {
				// 只有一个 FUNCTION_CODE 工具时缺标记的调用会被本地还原，另加一个让目标有歧义。
				source["tools"] = []any{definition, object{"type": "function", "name": "run_other", "parameters": object{"type": "object", "properties": object{"code": object{"type": "string"}}}}}
			}
...
func TestToolRepairDoesNotRetryCallbackFailure(t *testing.T) {
	...
	require.Equal(t, json.Number("20"), repairValue[object](t, response["usage"])["input_tokens"])
}
...
func TestToolRepairBindsOriginalRawCommand(t *testing.T) {
	source := testSource()
	// 另加一个 cmd 工具，避免缺标记的调用被本地唯一匹配还原。
	source["tools"] = []any{execCommandTestTool(), object{"type": "function", "name": "run_in_terminal", "parameters": object{"type": "object", "properties": object{"cmd": object{"type": "string"}}}}}
	cache := new(ReplayCache)
	...
	body := bridge.StreamWithToolRepair(context.Background(), io.NopCloser(strings.NewReader(sse(object{"type": "response.completed", "response": initial}))), func(context.Context, object, error) (object, error) {
		attempts++
		fixed := functionCodeTestNative(t, "exec_command", "rewritten command", "{\"description\":\"Run command\"}")
	...
	call := repairValue[object](t, output[0])
	require.Equal(t, original, functionCodeTestArguments(t, call)["cmd"])
	cached := cache.get("cmd-repair", "call_code")
	require.Equal(t, original, transportArguments(cached)["code"])
}

// 截断的 JSON 信封不是原始代码：允许模型整体重发，但能认出的工具名不能改。
func TestToolRepairReissuesBrokenEnvelope(t *testing.T) {
	broken := `{"name":"shell","arguments":{"cmd":"ls -la`
	for _, tc := range []struct {
		name      string
		corrected object
		ok        bool
	}{
		{name: "same_tool", corrected: repairCall("fixed", "List files", `{"name":"shell","arguments":{"cmd":"ls -la"}}`), ok: true},
		{name: "other_tool", corrected: repairCall("fixed", customTransportPrefix+"functions.exec", "text(1)"), ok: false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			cache := new(ReplayCache)
			_, bridge := repairBridge(t, cache)
			initial := repairResponse("resp", 10, 2, repairCall("bad", "List files", broken))
			body := bridge.StreamWithToolRepair(context.Background(), io.NopCloser(strings.NewReader(sse(object{"type": "response.completed", "response": initial}))), func(_ context.Context, _ object, validation error) (object, error) {
				require.ErrorContains(t, validation, "json_failure=unexpected_eof")
				return repairResponse("resp_fixed", 20, 3, tc.corrected), nil
			})
			events := repairEvents(t, body)
			last := events[len(events)-1]
			response := repairValue[object](t, last["response"])
			if !tc.ok {
				require.Equal(t, "response.failed", last["type"])
				require.Contains(t, repairValue[object](t, response["error"])["message"], "changed an operation")
				return
			}
			require.Equal(t, "response.completed", last["type"])
			call := repairValue[object](t, repairValue[[]any](t, response["output"])[0])
			require.Equal(t, "shell", call["name"])
			require.JSONEq(t, `{"cmd":"ls -la"}`, text(call["arguments"]))
		})
	}
}
