...
type EmailTask struct {
	...
	UserID   int64  // Authenticated recipient; zero for anonymous flows
}
...
func (s *EmailQueueService) processTask(workerID int, task EmailTask) {
	...
	case TaskTypeVerifyCode:
		if err := s.emailService.SendVerifyCodeForUser(ctx, task.Email, task.SiteName, task.UserID, task.Locale); err != nil {
...
func (s *EmailQueueService) EnqueueVerifyCode(email, siteName string, locale ...string) error {
	return s.enqueueVerifyCode(email, siteName, 0, locale...)
}

// EnqueueVerifyCodeForUser queues a verification email for an authenticated user.
func (s *EmailQueueService) EnqueueVerifyCodeForUser(email, siteName string, userID int64, locale ...string) error {
	return s.enqueueVerifyCode(email, siteName, userID, locale...)
}

func (s *EmailQueueService) enqueueVerifyCode(email, siteName string, userID int64, locale ...string) error {
	task := EmailTask{
		Email:    email,
		SiteName: siteName,
