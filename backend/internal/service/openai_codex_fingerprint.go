...
// codexFingerprintIDsStagedForAccount keeps the existing passthrough call
// path while using the same OAuth/account guard as the upstream helpers.
...
// applyCodexFingerprintRawPayload preserves the existing WebSocket caller
// contract while delegating to the upstream raw-body implementation.
