...
func openAIStreamFailureStatus(payload []byte, message string) int {
	...
	// response.failed is transported over an HTTP 200 stream. Keep the
	// downstream failover contract at 502; Ops classification is handled
	// separately below so capacity shedding does not become a client-visible
	// protocol change.
