...
func TestOpenAIBuildUpstreamRequestOpenAIPassthroughConvergesOAuthDeviceOnly(t *testing.T) {
	...
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, account, "client-session"), req.Header.Get("session_id"), "device 模式保留既有会话隔离（并按凭据身份命名空间收敛）")
	...
	// 会话类身份按所选凭据收敛：同一 client-session 经不同账号出站必须不同，
	// 避免上游把两个账号关联成同一会话。device 模式只保证 installation_id 不被再次改写。
	...
	require.Equal(t, scopedSession, headerMetadata["session_id"])
	...
	// 载荷侧的凭据身份收敛由上层 forwardOpenAIPassthrough 完成（见其对
	// applyCodexAccountIdentityClientMetadataRaw 的调用）；本构造器只负责出站头。
	// 因此在这一层，body 中的会话仍是客户端原值，device 模式也不改写它。
