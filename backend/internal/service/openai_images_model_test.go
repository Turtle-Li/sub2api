...
func TestOpenAIImages25MultiOutputPreservesDriverOverride(t *testing.T) {
	t.Setenv("SUB2API_IMAGES_MAIN_MODEL", " gpt-5.6-sol ")
	parsed := &OpenAIImagesRequest{Endpoint: openAIImagesGenerationsEndpoint, Model: "gpt-image-2.5-flare", Prompt: "draw two cups", N: 2}
	...
	require.Equal(t, parsed.Model, gjson.GetBytes(body, "tools.0.model").String())
	require.Contains(t, gjson.GetBytes(body, "instructions").String(), "requires exactly 2 final images")
	require.Contains(t, gjson.GetBytes(body, "instructions").String(), openAIImagesVerbatimPromptInstructions)
}
