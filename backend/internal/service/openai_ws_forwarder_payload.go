...
func (s *OpenAIGatewayService) buildOpenAIWSHeaders(
	...
	applyPinnedCodexTurnState(headers, account, routingModel)
	...
	if account != nil && account.Type == AccountTypeOAuth {
		// HTTP->WS 会复用 Forward 中预计算的 IDs；直连 WS 则使用入站连接当前轮次的 IDs。
		applyCodexFingerprintHeaders(headers, resolveCodexFingerprintIDsFromContext(account, c))
	}
