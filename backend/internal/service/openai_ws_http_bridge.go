	...
	if resp.StatusCode >= 400 {
		...
		if account.Platform == PlatformGrok {
			...
			if shouldFailover && (turn == 1 || resp.StatusCode == http.StatusTooManyRequests) {
				return nil, newOpenAIUpstreamFailoverError(account, resp.StatusCode, resp.Header, respBody, upstreamMsg, false)
			}
	...
	for scanner.Scan() {
		...
		if eventType == "error" || eventType == "response.failed" {
			...
			if eventType == "error" {
				...
				statusCode = openAIWSErrorHTTPStatusFromEventFields(errCodeRaw, errTypeRaw, errMessage)
			...
			} else if account.Platform != PlatformGrok && eventType == "error" && shouldFailover && !requestScopedCapacity {
			...
			if !wroteDownstream && shouldFailover && (turn == 1 || statusCode == http.StatusTooManyRequests) {
				if account.Platform == PlatformGrok {
					return nil, newOpenAIUpstreamFailoverError(account, statusCode, resp.Header, upstreamMessage, errMessage, false)
				}
