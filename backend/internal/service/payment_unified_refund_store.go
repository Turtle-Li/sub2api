...
type unifiedRefundAttempt struct {
	...
	ProviderStatus, FailureCode                                                              string
...
func loadUnifiedRefundAttempt(ctx context.Context, client *dbent.Client, orderID int64, refundNo string) (*unifiedRefundAttempt, error) {
	...
	var valuation, providerUpdated sql.NullTime
	...
	if providerUpdated.Valid {
		value := providerUpdated.Time
		a.ProviderUpdatedAt = &value
	}
	return a, err
}
...
// unifiedRefundOrderHasOtherReview excludes the current attempt so an explicit,
// trusted recovery command can resolve that attempt's own fence without
// erasing a separate correlation or signed-event conflict on the same order.
func unifiedRefundOrderHasOtherReview(ctx context.Context, client *dbent.Client, orderID int64, productRefundNo string) (bool, error) {
	rows, err := client.QueryContext(ctx, `SELECT
	 (SELECT COUNT(*) FROM unified_payment_refund_attempts
	    WHERE order_id = $1 AND product_refund_no <> $2 AND needs_manual_review = TRUE)
	 + (SELECT COUNT(*) FROM unified_payment_refund_events WHERE order_id = $1
	    AND action IN ('UNIFIED_REFUND_UNCORRELATED', 'UNIFIED_PAYMENT_EVENT_REJECTED'))`, orderID, productRefundNo)
	if err != nil {
		return false, err
	}
	defer func() { _ = rows.Close() }()
...
func saveUnifiedRefundAttempt(ctx context.Context, client *dbent.Client, a *unifiedRefundAttempt) error {
	_, err := client.ExecContext(ctx, `UPDATE unified_payment_refund_attempts SET
	 ...
	 provider_status=$6, failure_code=$7, provider_updated_at=$8,
	 needs_manual_review=$9, entitlement_reserved=$10, updated_at=CURRENT_TIMESTAMP WHERE product_refund_no=$1`,
		a.ProductRefundNo, a.Status, nullableUnifiedRefundID(a.RefundRequestID),
		nullableUnifiedRefundID(a.ChannelOutRefundNo), nullableUnifiedRefundID(a.ProviderRefundID),
		nullableUnifiedRefundID(a.ProviderStatus), nullableUnifiedRefundID(a.FailureCode), a.ProviderUpdatedAt,
		a.NeedsManualReview, a.EntitlementReserved)
	return err
}
