...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	if resetTimestamp == "" {
		...
		case PlatformOpenAI:
			// 尝试解析 OpenAI 的 usage_limit_reached / OpenCode Go 用量错误
		...
		case PlatformDeepseek:
			// DeepSeek 标识的 OpenCode Go 账号复用同一个 OpenAI-compatible
			// 用量错误格式；仅对明确匹配 OpenCode Go 端点的账号启用解析，
			// 避免改变普通 DeepSeek 账号的 429 回退策略。
