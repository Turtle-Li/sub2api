	...
	for _, id := range input.SubscriptionIDs {
		...
		if err == nil {
			...
			err = s.withSubscriptionUpdateTx(ctx, func(txCtx context.Context) error {
				// The item transaction owns the mutation boundary. Suppress the
				// single-item services' pre-commit invalidation and invalidate once
				// below, after a successful commit.
				txCtx = withDeferredSubscriptionCacheInvalidation(txCtx)
