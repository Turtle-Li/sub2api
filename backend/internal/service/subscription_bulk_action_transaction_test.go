...
func TestBulkSubscriptionAction_InvalidatesCacheOnlyAfterCommit(t *testing.T) {
	...
	key := subCacheKey(repo.committed.UserID, repo.committed.GroupID)
	initialEpoch := svc.subscriptionCacheEpoch(key)

	var epochBeforeCommit uint64
	repo.beforeCommit = func() {
		epochBeforeCommit = svc.subscriptionCacheEpoch(key)
	}
	mock.ExpectBegin()
	mock.ExpectCommit()
	...
	require.Equal(t, initialEpoch, epochBeforeCommit, "cache invalidation must not advance before the transaction commits")
	require.Greater(t, svc.subscriptionCacheEpoch(key), initialEpoch, "the committed mutation must invalidate the old cache generation")
