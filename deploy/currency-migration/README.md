# Internal-credit rescaling cutover (2026-09-12)

> Historical terminology: `USD`/`CNY` in this directory are implementation,
> source-price and migration markers. Wallet balances, quotas, subscriptions and
> usage are generic internal units, not fiat-denominated assets. Payment, refund,
> invoice and upstream provider-cost facts retain their external currency. This
> migration has completed; do not rerun it.

This was an explicit operator migration, not an application startup migration.
Approved scale factor: **6.75**. The default compatibility basis remained `USD`
until the transaction.

**Owner-approved execution:** future recharge is **1 CNY actually paid → 1
internal unit credited**, so use `recharge_factor=1`. On September 12 the owner
explicitly prioritized uninterrupted API use and accepted temporary
undercharging. The [online execution packet](ONLINE_CUTOVER_20260912.md) records
the completed run; the stopped-writer steps below are the earlier alternative,
not authorization for another execution. Never activate the HTTP admission
fence from this historical packet.

## Required gates

1. Review the exact application commit with Claude, run backend unit tests and frontend checks, and deploy the same reviewed image to both production application nodes with existing role-preserving release receivers. Keep the default `USD` compatibility policy during rollout.
...
6. Apply a single transaction that snapshots only selected monetary fields, converts wallet data, sets the `CNY` compatibility policy and records a unique migration marker. A repeat must fail. Purge only the documented wallet/auth/key/platform-quota cache namespaces while writers are stopped; do not `FLUSHALL` shared Redis.
7. Start the reviewed application generation with fresh compatibility-policy caches while traffic-state remains draining. Restore the recorded background-owner role; run canaries over the actual loopback socket inside the application container with the existing monitor token plus normal API credentials. This permits testing while public traffic remains fenced; forwarded headers never qualify. Restore accepting state only after canaries pass. Run an isolated wallet debit and subscription debit canary, confirm corresponding usage-row markers and before/after balance/quota. Remove only the dedicated test fixtures after preserving evidence. Reopen admissions after both pass.

## Conversion scope

- Users: `balance`, `frozen_balance`, `total_recharged`; fixed balance notification thresholds only.
- Wallet API keys: quota limits, accumulated quota, 5h/1d/7d rate limits and
  usage. Subscription-bound keys are not rescaled and retain their `USD`
  reference marker.
...
- Existing registration/first-bind/default balance settings and fixed
  notification defaults. Group/model discount multipliers are not converted.
  `BALANCE_RECHARGE_MULTIPLIER` is internal units credited per CNY actually paid,
  not a discount: its migration handling and preset bonuses depended on the
  pending owner decision for new recharges (`recharge_factor=6.75` preserved the
  earlier retail purchasing power; `1` retained nominal internal-unit amounts).
  The script refuses an unspecified factor. Update any preset descriptions that
  embed old bonus numbers after reviewing the exact catalog. Rehearsals use
...
- A zero dirty set before stopping does not replace checking again after stop. There is no verified standalone synchronous flusher command in this packet. If dirty snapshots remain, halt and reconcile/flush them under the pre-migration compatibility policy; never delete them to pass the gate.
- Provision dedicated wallet and subscription fixtures before final execution.
  A wallet fixture with its own platform quotas must produce a nonzero
  `CNY`-marked usage row whose internal-unit `actual_cost` matches the wallet
  decrease and applicable key/platform usage increase. A subscription fixture
  must produce a nonzero `USD`-marked usage row, increase subscription/key usage,
  and leave its wallet unchanged. Verify DB/cache parity after asynchronous
  flushing. Do not use real payment orders or modify an ordinary subscriber to
  manufacture a test fixture.
