...
# Install the candidate-side replica watchdog only after pre-sync is ready.
set -Eeuo pipefail
...
SERVICE_NAME=sub2api-redis-streaming-watchdog.service
TIMER_NAME=sub2api-redis-streaming-watchdog.timer
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --locked) LOCKED=true ;;
    --help|-h) echo "Usage: install-redis-streaming-watchdog.sh"; exit 0 ;;
...
for command_name in cp install mktemp systemctl systemd-analyze; do command -v "$command_name" >/dev/null 2>&1 || die "$command_name is required"; done

runner_source="$SCRIPT_DIR/sub2api-redis-streaming-watchdog.sh"
service_source="$SCRIPT_DIR/$SERVICE_NAME"
timer_source="$SCRIPT_DIR/$TIMER_NAME"
runner_target=/usr/local/libexec/sub2api-redis-streaming-watchdog.sh
...
backup_file runner "$runner_target"; backup_file service "$service_target"; backup_file timer "$timer_target"
rollback_armed=true
cleanup() {
  local status=$?
  trap - EXIT
  if [ "$status" -ne 0 ] && [ "$rollback_armed" = true ]; then
    ...
    restore_file runner "$runner_target"; restore_file service "$service_target"; restore_file timer "$timer_target"
...
SUB2API_REDIS_STREAMING_LOCK_HELD=true "$runner_target" >/dev/null
grep -q '^state=healthy .*sync_phase=incremental ' /run/sub2api-redis-streaming.status || die "watchdog did not report incremental readiness"
...
rm -rf "$backup_dir"
trap - EXIT
echo "Installed Redis streaming watchdog."
