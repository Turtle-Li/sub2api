...
case "$LOCAL_BIND" in 0.0.0.0|::|'') echo "ERROR: wildcard local bind is forbidden" >&2; exit 1 ;; esac
...
for value in "$SSH_PORT" "$LOCAL_PORT" "$REMOTE_PORT"; do
  ...
  [ "$value" -ge 1 ] && [ "$value" -le 65535 ] || { echo "ERROR: port is outside 1-65535" >&2; exit 1; }
done
