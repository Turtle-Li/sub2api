...
统一支付支持管理员对普通余额订单发起异步退款：受理和超时均保持待确认，只有可信成功结果
才按管理员的“扣回余额”选择执行一次扣回。赠送权益、并发升级及订阅退款仍需单独人工处理；
本次不新增用户自助退款权限。完整契约和演练边界见 [统一支付接入说明](UNIFIED_PAYMENT_INTEGRATION.md)。
