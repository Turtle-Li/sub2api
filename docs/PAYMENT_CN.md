...
Sub2API 内置支付系统，支持用户自助充值；也可以把支付宝、微信收款路由到独立部署的统一支付服务。
...
| **统一支付服务** | 支付宝、微信支付 | 统一接收通道通知，验签后以产品 Webhook 通知 Sub2；Sub2 仍保有业务订单和发货真相 |

> 支付宝官方 / 微信官方、易支付与统一支付可以同时存在，但前台始终只展示 `支付宝`、`微信支付` 两个可见按钮。管理员可在支付设置中分别选择按钮来源；统一支付来源不需要创建本地服务商实例。
...
2. 开启 **启用支付**
3. 配置基本参数（金额范围、超时时间等）
4. 如果使用传统直连/易支付，在 **服务商管理** 中添加服务商实例；如果使用统一支付，确认服务器接入状态为“已配置”并选择统一支付路由
5. 用户即可在前端页面进行充值
...
在同一页的 **支付页活动 Banner** 区域可启用一个 Banner，用于活动或广告跳转。只需填写标题、跳转链接（支持 `http(s)` 或站内路径），说明、按钮文字和图片均为可选；前台统一使用系统样式，不开放主题色、图标、角标或轮播配置。启用时必须填写标题和链接，链接会在服务端校验。
...
- **支付宝**：后台启用后，可路由到 `支付宝官方`、`易支付支付宝` 或 `统一支付支付宝`
- **微信支付**：后台启用后，可路由到 `微信官方`、`易支付微信` 或 `统一支付微信`
- 同一个可见支付方式在同一时刻只能路由到一个来源
- 来源为“自动选择”时，优先使用服务器已配置的统一支付方式，否则沿用传统服务商选择
- 明确选择官方或易支付时，下单固定在该来源；读取配置失败时停止下单
- 旧版可见按钮 `enabled` 字段仅为兼容字段，不承担路由开关语义

统一支付的签名密钥由服务器和 Vault 管理，后台只读展示可用方式，不接收私钥。
统一支付支持管理员对普通余额订单发起异步退款，也支持受上限约束的连续部分退款。
`refund_amount` 记录已可信成功的累计金额，`refund_requested_amount` 记录唯一在途申请；累计金额
不得超过订单可退款金额，待确认/未知申请完成前不能再发起下一笔。只有可信成功结果才按管理员的
“扣回余额”选择执行一次扣回。余额档位 bonus 在购买快照证明已折入到账金额时随余额一并回收；
余额不足、并发升级、订阅 bonus、reset card 及订阅部分退款均转人工权益复核。本次不新增无限制的
用户自助退款权限。完整契约和演练边界见 [统一支付接入说明](UNIFIED_PAYMENT_INTEGRATION.md)。
...
在 **支付管理 → 订阅套餐** 中可以创建多个在售套餐，并配置：原价/售价（前台展示
划线价和折扣）、天/周/月/季度/年度有效期、功能说明，以及购买后赠送的余额和额度重置
次数。可额外勾选“推荐套餐”，前台在对应周期显示统一的推荐样式；同一可见周期有多个推荐时，仅突出显示排序靠前的第一个，未手动推荐时保留原有折扣推荐逻辑；该标记不影响价格、排序或履约。季度和年度当前按固定的 90/365 天履约，不包含自动续费。

支付订单会保存套餐价格、周期和权益快照；之后修改套餐不会影响已创建订单。支付成功后
服务端会在同一履约事务中开通订阅、发放赠送余额和重置次数，并记录幂等审计，重复回调
或手动重试不会重复到账。完整字段和扩展边界见
[`PAYMENT_COMMERCE_DESIGN.md`](PAYMENT_COMMERCE_DESIGN.md)。
...
启用 `unified_payment` 后，前台仍显示原有“支付宝”和“微信支付”按钮，但创建支付单会调用统一支付服务，
...
用户选择充值金额和支付方式
  ...
  支付回调验签 → 订单 PAID
       │
       ▼
  余额订单：自动充值到账；订阅订单：开通套餐并发放权益 → 订单 COMPLETED
```
...
| `PAID` | 已支付，等待充值到账 |
| `COMPLETED` | 已完成，余额已到账或订阅权益已发放 |
| `EXPIRED` | 已过期，超时未支付 |
| `CANCELLED` | 已取消，用户主动取消 |
| `FAILED` | 充值失败，可管理员重试 |
| `REFUND_REQUESTED` | 已申请退款 |
| `REFUNDING` | 退款处理中 |
| `PARTIALLY_REFUNDED` | 部分退款已完成，仍有可退款余额 |
| `REFUNDED` | 已退款 |
