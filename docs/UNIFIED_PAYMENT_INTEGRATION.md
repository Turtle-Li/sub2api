...
> 2026-09-22 本地取消及晚到付款自动退款的后续契约见 `operations/PAYMENT_LOCAL_CANCELLATION_20260922.md`，已完成本地验证但尚未发布，不能据此推断线上版本已具备能力。
...
迁移 `237_unified_payment_refund_attempts.sql` 新增退款尝试与追加式结果证据；迁移
`240_payment_refund_reclaim.sql` 另外记录订单的累计已结算退款与当前在途申请金额，并把
订阅 reset-card 发放记录关联回支付订单。原有 `(order_id, action)` 发货审计唯一约束保持不变。
执行流程：
...
2026-09-19 产品规则更新：一个订单最多成功退款一次，部分退款成功后也不再接受新退款；失败申请仍可重试，已有在途申请继续按同一退款号恢复。历史累计金额不得超过
订单可退款金额；`refund_amount` 是已确认成功的累计金额，`refund_requested_amount` 是当前
在途申请，通道请求金额是累计目标的整数舍入差值。待确认、金额/方式冲突或失败状态会阻断
下一次自动处理，直到查单或人工处理完成。用户自助退款仍受服务商开关、余额产品和权益安全
栅栏限制，不因管理员接口而开放。

余额档位的 bonus 必须在不可变购买快照中证明已折入 `PaymentOrder.amount`，普通余额扣回会
一并回收；余额不足时只按实际可用余额扣回并转人工复核。订阅 bonus、并发升级、reset card
以及订阅的部分退款没有可安全逆向的权益流水，均拒绝自动退款并要求人工撤销/核对；reset
grant 的订单关联只提供可审计证据，不会在未知状态下自动删除权益。所有通道金额采用整数比例
和整数舍入，旧产品浮点 DTO 只在边界转成最小单位，不参与新的通道退款运算。
...
剩余边界：真实支付宝/微信通道与税务发票仍需按 live 验收门槛单独验证；不可逆订阅权益和
订阅部分退款继续保留人工复核限制。
...
生产通道和产品绑定就绪后，在管理后台“支付订单”的“管理员小额实测”选择 1 分或 2 分，再选支付宝或微信。接口要求当前有效管理员会话及已有的二次认证；普通用户购买设置继续关闭，不需要临时放开购买入口。
