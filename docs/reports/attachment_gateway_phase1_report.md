...
生产只读复核确认当前 Caddy 门为十进制 16,000,000 B。13,749,545 B 的真实 HTTP
请求已进入 Sub2 并降到 2,945,577 B；超过 16,000,000 B 的请求仍会在进入优化器前
被拒绝。本模块降低的是已进入 Sub2 后的上游 payload、带宽保护和 failover 重放压力。
...
仓库内 `deploy/Caddyfile` 的示例上限为 256 MiB。2026-07-20 生产当前配置为：
...
并在 `Content-Length > 16000000` 时提前返回 413。13,749,545 B 的 canary 已成功到达
Sub2；16,000,000 B 以上仍无法由本模块补救。
...
9. 最终 linux/amd64 Alpine 功能已验证，但 Rosetta/QEMU 数据不能外推为生产 Xeon
   p95/p99。
10. Sub2 内优化无法绕过 Caddy 当前前置 16,000,000 B 413。
