...
> **后续状态说明**：本文保留最初离线调研基线。此后 Phase 1 已实现并以显式白名单
> 进入 scoped 生产 rewrite，Responses Caddy 入站已独立提高到 256,000,000 B，实际
> 上游仍限制为 16,000,000 B。最新生产数据、问题分级和结论见
> `attachment_gateway_current_status.md`；下文“未进入生产”的表述仅描述当时阶段。
...
当时 POC **不应直接接入生产核心链路**；它适合作为可合并的 `experiments/` 证据。
后续生产候选已补齐 CPU、缓存、范围控制和入口/出口护栏，但仍未全量开启。
