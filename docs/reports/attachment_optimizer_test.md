...
结论：本文保留 Phase 1 离线 POC 基线；最新生产 method 0 canary、256,000,000 B
Responses 入站、16,000,000 B 上游保险和真实模型质量结果见
`attachment_gateway_phase1_report.md`。
...
这是当时离线 POC 的边界。生产随后已把 Responses 专用 Caddy 入站提高到
256,000,000 B，并用 17,485,303 B 真实请求验证先进入 Sub2、压到 3,985,423 B、
再由模型返回 200；非 Responses 入口仍为 16,000,000 B。
...
- **视觉质量：通过当前 canary。** 离线 SSIM/OCR 通过；后续真实模型已验证
  PNG/JPEG/WebP 语义及聚合小 UI 图中文 OCR 全部正确。
- **性能：有条件通过。** 单图冷压缩约 1.35 s，必须限并发、设超时和 fail-open；zopflipng 不可在线。
- **入口 413：已独立放宽。** Responses 原始入站为 256,000,000 B；压缩后实际转发仍
  受 16,000,000 B 硬保险保护。17,485,303 B 的生产 HTTP canary 已降到
  3,985,423 B 并返回 200。
