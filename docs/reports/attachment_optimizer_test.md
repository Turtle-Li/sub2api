...
结论：图片压缩方向在体积上成立；当前同步冷压缩 CPU 成本偏高，且 Sub2 内优化不能绕过 Caddy 前置 10 MB 413。
...
按项目 Caddy 快照的十进制 `10,000,000 B` 门限：
...
```text
Client → Caddy 10 MB gate → Sub2 optimizer → OpenAI
```
...
- **生产 413：未解决。** Sub2 内优化位于 Caddy 后，无法绕过当前 10 MB gate。
