<template>
  <div class="flex items-start gap-2 rounded border p-2"
    ...
    <!-- Token mode: context range + prices in the selected authored unit -->
    <template v-if="mode === 'token'">
      <div class="pricing-interval-grid grid min-w-0 flex-1 gap-2">
        ...
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.inputPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ priceUnit }}/M</span></label>
        ...
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.outputPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ priceUnit }}/M</span></label>
          <input :value="interval.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWrite5mPriceShort') }} <span class="text-gray-300">{{ priceUnit }}/M</span></label>
          <input :value="interval.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWrite1hPriceShort') }} <span class="text-gray-300">{{ priceUnit }}/M</span></label>
          <input :value="interval.cache_write_1h_price" @input="emitField('cache_write_1h_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheReadPriceShort') }} <span class="text-gray-300">{{ priceUnit }}/M</span></label>
    ...
    <template v-else>
      ...
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.perRequestPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ priceUnit }}</span></label>
...
import { normalizePricingCurrency, type IntervalFormEntry } from './types'
...
const priceUnit = computed(() => normalizePricingCurrency(props.currency))
