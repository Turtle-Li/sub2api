export default {
    settings: {
      ...
      pricingCurrency: {
        title: 'Wallet settlement currency',
        description: 'Shows the active wallet unit and records the approved USD to CNY migration rate.',
        settlementCurrency: 'Active settlement currency',
        settlementCurrencyHint: 'Read-only here. Changing USD/CNY requires the owner-controlled wallet and quota migration transaction.',
        usdToCnyRate: 'Approved USD → CNY migration rate',
        rateHint: 'This is the approved migration rate, not a live exchange-rate feed.',
        migrationNotice: 'This screen cannot change the wallet currency. A USD/CNY transition must migrate balances and key quotas in one controlled transaction; ordinary saves only update the rate for the active currency.',
        subscriptionScope: 'Subscription quotas and payment-order currencies remain USD.',
        invalidRate: 'Enter a finite positive migration rate.',
        loadFailed: 'Failed to load pricing currency settings',
        saveFailed: 'Failed to save pricing currency settings',
        saved: 'Pricing currency settings saved',
      },
      attachmentGatewayR2: {
        title: 'Attachment Gateway R2',
        description: 'Configure a dedicated private R2 bucket for URL externalization after Responses image optimization. Backup and async-image credentials are not reused.',
        safetyTitle: 'Independent config with rollout safeguards',
        safetyHint: 'Enabling this storage only makes R2 available to Attachment Gateway. It does not enable image optimization, URL rewriting, unscoped users, or change HTTP/WS routes. The server-side experiment gate, allowlist, and rewrite mode must still all be enabled.',
        enabled: 'Enable Attachment R2 storage',
        enabledHint: 'When disabled, URL externalization is skipped safely and optimized images remain inline Base64.',
        endpoint: 'R2 S3 API Endpoint',
        region: 'Region',
        bucket: 'Bucket name',
        prefix: 'Object prefix (optional)',
        prefixHint: 'For example sub2api/; content-hash objects remain under an attachments/ subpath.',
        accessKeyId: 'Access Key ID',
        secretAccessKey: 'Secret Access Key',
        secretConfigured: 'Secret configured; leave blank to keep it',
        secretHint: 'The secret is encrypted at rest and never returned by the API. Leaving it blank uses the saved secret for save or test.',
        presignExpiry: 'Signed URL expiry (minutes)',
        presignExpiryHint: 'Allowed range: 5–10080 minutes. 60 minutes is recommended; the process cache never outlives the URL safety window.',
        forcePathStyle: 'Use path-style access (normally off for Cloudflare R2)',
        permissionHint: 'Use an Object Read & Write token scoped only to this bucket. Admin Read & Write is unnecessary, and the bucket should remain private.',
        probeHint: 'Test Connection writes, reads, and deletes one tiny probe object to verify real permissions. Submitted test values are not saved.',
        lifecycleHint: 'Add an R2 lifecycle rule for this prefix. During the experiment, delete objects after 1–7 days so user attachments are not retained indefinitely.',
        testConnection: 'Test read/write connection',
        testSuccess: 'R2 read/write connection succeeded',
        testFailed: 'R2 read/write connection failed',
        saved: 'Attachment Gateway R2 config saved and hot-reloaded',
        statusUnconfigured: 'Not configured',
        statusDisabled: 'Configured / disabled',
        statusEnabled: 'Configured / enabled',
      },
      features: {
        channelMonitor: {
      ...
      payment: {
        ...
        unifiedHint: 'After server setup, choose the payment route for Alipay and WeChat here. Signing keys are securely managed by the server.',
        unifiedMethods: 'Available methods: {methods}',
        unifiedNone: 'none',
        unifiedAlipayRoute: 'Alipay route',
        unifiedWechatRoute: 'WeChat route',
        unifiedAutomaticRoute: 'Automatic (prefer unified payment)',
        unifiedCallbackHint: 'Callback path: /api/v1/payment/webhook/unified. Credit is applied only after the unified payment service confirms the payment.',
        ...
        bannerTitle: 'Payment page promotion banner',
        bannerHint: 'Use this for an activity or advertising click-through. Only content and destination are configurable; the frontend owns the visual style.',
        bannerEnabled: 'Enable banner',
        bannerTitleField: 'Title',
        bannerTitlePlaceholder: 'For example: New users get 10% extra balance',
        bannerDescription: 'Description',
        bannerDescriptionPlaceholder: 'One-line explanation (optional)',
        bannerLink: 'Destination URL',
        bannerLinkPlaceholder: 'https://example.com/activity or /announcements/1',
        bannerLinkHint: 'HTTP(S) URLs and site-relative paths are supported; required when enabled.',
        bannerButtonText: 'Button text',
        bannerButtonPlaceholder: 'View activity',
        bannerImage: 'Banner image',
        bannerImagePlaceholder: 'Upload an activity image (optional)',
        bannerImageFormatError: 'Upload a raster image such as PNG, JPEG, WebP or GIF. SVG is not supported.',
        bannerImageHint: 'A fixed system illustration is used when no image is uploaded; maximum 300 KB.',
