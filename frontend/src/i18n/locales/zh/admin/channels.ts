export default {
    ...
    subscriptions: {
      ...
      remainingResetCount: '剩余 {count} 次',
      grantResetCards: '发放重置卡',
      grantResetCardsShort: '发次数',
      grantResetCardsTitle: '按分组发放重置卡',
      grantResetCardsDesc: '为所选订阅分组中当前有效的用户发放重置卡。重复发放会叠加，用户使用时优先消耗最早到期的卡。',
      grantResetCardsToUserTitle: '给用户发放重置卡',
      grantResetCardsToUserDesc: '只为列表中选中的这一条用户订阅增加重置次数，不会影响该用户的其他订阅。',
      resetCardsForm: {
        target: '发放对象',
        targetHint: '重置卡仅绑定到上方这一条用户订阅',
        groups: '订阅分组',
        groupsHint: '只会发给所选分组中当前有效且未过期的订阅用户',
        count: '每位用户发放数量',
        directCount: '发放数量',
        countHint: '每次最多 1000 张，可重复发放叠加',
        expiresAt: '卡片到期时间',
        expiresAtHint: '默认两周后到期；到期后未使用的次数自动失效'
      },
      pleaseSelectResetCardGroups: '请至少选择一个订阅分组',
      invalidResetCardCount: '发放数量必须在 1 到 1000 之间',
      invalidResetCardExpiry: '卡片到期时间必须晚于当前时间，且不超过 2099-12-31',
      resetCardsGranted: '已向 {recipients} 份有效订阅发放 {cards} 张重置卡',
      resetCardsGrantedToUser: '已向 {user} 的“{group}”订阅发放 {cards} 张重置卡',
      resetCardsNoRecipients: '所选分组中没有当前有效订阅，未发放重置卡',
      failedToGrantResetCards: '发放重置卡失败',
