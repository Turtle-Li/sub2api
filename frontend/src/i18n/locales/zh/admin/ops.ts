export default {
    ops: {
      ...
      network: {
        ...
        manualLimit: '手动上限',
        suspectedShort: '疑似拥塞',
        disabledHint: '已在设置中关闭近实时监控。',
        empty: '暂无带宽采样数据',
        loadFailed: '加载带宽数据失败',
        settingsLoadFailed: '加载带宽设置失败',
        settingsSaveSuccess: '带宽设置已保存',
        settingsSaveFailed: '保存带宽设置失败',
        invalidSettings: '请检查带宽上限和异常重试参数。',
        enableMonitor: '启用带宽监控',
        enableMonitorHint: '每 5 秒从服务器网卡计数器采样一次入站/出站流量。',
        retryProtection: '实验：异常重试保护',
        retryProtectionHint: '仅在已配置入站或出站上限，且任一方向利用率超过阈值时，限制短时间内重复提交的相同大请求。',
        retryRecommendedTitle: '推荐设置：适合多数小带宽服务器',
        retryRecommendedBody: '建议先使用 90% / 60 秒 / 允许 1 次重试。它不会限制偶发的大请求，只会在带宽已经接近上限时，识别同一个大请求体的高频重复提交。',
        retryUseRecommended: '恢复推荐值',
        retryCurrentStrategy: '当前策略',
        retryPresetRecommended: '推荐配置',
        retryPresetCustom: '自定义配置',
        retryStrategySummary: '带宽达到 {trigger}% 后，观察 {window} 秒内的相同大请求，允许重复 {repeats} 次。',
        retryEstimateSummary: '按{direction} {limit} Mbps 估算：约 {candidate} 起计算指纹；累计重复超过 {budget} 后，第 {count} 次起才可能限制。',
        retryAdvancedSettings: '高级设置',
        retryAdvancedHint: '通常无需修改。只有出现误识别，或需要针对服务器性能调优时再调整。',
        retryFormula: '公式：重复预算 = 较小带宽上限 × 1,000,000 ÷ 8 × 窗口秒数 × 触发百分比；识别门槛 = max(最低识别门槛, 重复预算 ÷ (允许次数 + 2))。',
        retryBudget: '重复预算',
        retryBudgetHint: '按 {direction} {limit} Mbps 计算',
        retryCandidate: '实际识别门槛',
        retryCandidateHint: '动态门槛：{dynamic}',
        retryBlockRule: '限制规则',
        retryBlockAttempt: '第 {count} 次起可能限制',
        retryBlockRuleHint: '还必须满足带宽拥塞、同请求体、累计大小超过预算。',
        retryNeedsLimitHint: '先填写入站或出站带宽上限后，这里会显示本次设置实际换算出的预算和识别门槛。',
        retryTriggerPercent: '带宽超过多少才启用',
        retryMinBody: '最低识别门槛',
        retryMinBodyHint: '只决定从多大的请求开始识别，不是请求大小上限。',
        retryAutoBodyHint: '“最低识别门槛”不是固定限制值，它只是地板；实际从多大的请求开始计算指纹，会按上面的公式自动变大。当前地板：{min} MB。',
        retryProtectionWarning: '低配置服务器或高并发大请求场景下可能增加 CPU 消耗；建议先保持推荐设置，出现影响时再关闭或调整高级参数。',
        retryWindow: '观察多久内的重复',
        retryMaxRepeats: '允许重复几次',
        limitPlaceholder: '留空则不按上限告警',
        ifacePlaceholder: '自动识别默认网卡',
        ifaceAuto: '自动识别（当前：{iface}）',
        ifaceUnknown: '未知',
        ifaceHint: '通常保持自动即可；只有多网卡或容器部署识别不准时才需要手动选择。',
        defaultIface: '默认',
        manualLimitHint: '带宽告警只使用这里配置的上限；未配置上限时，仅展示近实时出入站采样，不触发带宽利用率告警。',
        avg1m: '1m 均值',
        avg5m: '5m 均值',
        peak1h: '1h 峰值',
        packetWarning: '检测到网卡丢包或错误，可能存在网络层异常。',
        status: {
          ok: '正常',
          warning: '预警',
          critical: '严重',
