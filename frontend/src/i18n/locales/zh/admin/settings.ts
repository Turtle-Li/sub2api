export default {
    settings: {
      ...
      pricingCurrency: {
        title: '钱包结算币种',
        description: '显示当前钱包结算单位，并记录已批准的 USD 到 CNY 迁移汇率。',
        settlementCurrency: '当前结算币种',
        settlementCurrencyHint: '此处只读。USD/CNY 切换必须通过受控的钱包与 Key 配额迁移事务完成。',
        usdToCnyRate: '已批准的 USD → CNY 迁移汇率',
        rateHint: '这是迁移核准汇率，不是实时汇率源。',
        migrationNotice: '本页不能切换钱包币种。USD/CNY 切换必须在一个受控事务中同步迁移余额和 Key 配额；普通保存只可更新当前币种的汇率。',
        subscriptionScope: '订阅额度和支付订单币种继续保持 USD。',
        invalidRate: '请输入有限且大于 0 的迁移汇率。',
        loadFailed: '加载定价币种设置失败',
        saveFailed: '保存定价币种设置失败',
        saved: '定价币种设置已保存',
      },
      attachmentGatewayR2: {
        title: 'Attachment Gateway 专用 R2',
        description: '为 Responses 图片压缩后的 URL 化配置独立私有 R2。此处不会复用数据库备份或异步生图的对象存储凭证。',
        safetyTitle: '独立配置与灰度保护',
        safetyHint: '启用此存储只表示 R2 可供附件网关使用；不会自动开启图片优化、URL 替换、全量用户或修改 HTTP/WS 路由。服务器侧实验开关、白名单和 rewrite 模式仍必须同时满足。',
        enabled: '启用 Attachment R2 存储',
        enabledHint: '关闭时 URL 化会安全跳过，压缩后的图片仍以内联 Base64 转发。',
        endpoint: 'R2 S3 API Endpoint',
        region: 'Region',
        bucket: '存储桶名称',
        prefix: '对象前缀（可选）',
        prefixHint: '例如 sub2api/；实际 hash 对象会继续放在 attachments/ 子路径。',
        accessKeyId: 'Access Key ID',
        secretAccessKey: 'Secret Access Key',
        secretConfigured: '密钥已配置，留空以保留',
        secretHint: '密钥加密保存且不会从接口返回。留空保存或测试时会使用现有密钥。',
        presignExpiry: '签名 URL 有效期（分钟）',
        presignExpiryHint: '允许 5–10080 分钟，建议 60 分钟；进程缓存不会超过签名 URL 的安全有效期。',
        forcePathStyle: '使用 Path-style 访问（Cloudflare R2 通常不勾选）',
        permissionHint: '建议使用只限定到该存储桶的 Object Read & Write Token；不需要管理员读写权限，存储桶保持私有。',
        probeHint: '“测试连接”会写入、读取并删除一个极小探针对象，以验证真实读写权限；测试配置不会被保存。',
        lifecycleHint: '请在 R2 中为该前缀设置生命周期规则；实验阶段建议 1–7 天自动删除，避免长期保留用户附件。',
        testConnection: '测试读写连接',
        testSuccess: 'R2 读写连接正常',
        testFailed: 'R2 读写连接失败',
        saved: 'Attachment Gateway R2 配置已保存并热加载',
        statusUnconfigured: '未配置',
        statusDisabled: '已配置 / 未启用',
        statusEnabled: '已配置 / 已启用',
      },
      features: {
        channelMonitor: {
          title: '渠道监控',
      ...
      payment: {
        ...
        unifiedHint: '服务端完成接入配置后，可在此选择支付宝和微信的支付通道。签名密钥由服务端安全托管。',
        unifiedMethods: '可用方式：{methods}',
        unifiedNone: '无',
        unifiedAlipayRoute: '支付宝路由',
        unifiedWechatRoute: '微信路由',
        unifiedAutomaticRoute: '自动选择（优先统一支付）',
        unifiedCallbackHint: '回调路径：/api/v1/payment/webhook/unified。支付结果须经统一支付服务确认后才会到账。',
        ...
        bannerTitle: '支付页活动 Banner',
        bannerHint: '用于活动或广告跳转。只配置内容和链接，前台样式固定，避免后台出现过多自定义项。',
        bannerEnabled: '启用 Banner',
        bannerTitleField: '标题',
        bannerTitlePlaceholder: '例如：新用户充值赠送 10%',
        bannerDescription: '说明',
        bannerDescriptionPlaceholder: '一句话说明活动内容（可选）',
        bannerLink: '跳转链接',
        bannerLinkPlaceholder: 'https://example.com/activity 或 /announcements/1',
        bannerLinkHint: '支持 http(s) 链接或站内路径；启用时必须填写。',
        bannerButtonText: '按钮文字',
        bannerButtonPlaceholder: '查看活动',
        bannerImage: 'Banner 图片',
        bannerImagePlaceholder: '上传活动图片（可选）',
        bannerImageFormatError: '请上传 PNG、JPEG、WebP 或 GIF 等位图，暂不支持 SVG。',
        bannerImageHint: '不上传图片时使用系统默认图形；图片最大 300 KB。',
        providerEasypay: '易支付',
