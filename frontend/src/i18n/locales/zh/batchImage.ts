export default {
  batchImage: {
    ...
    itemResult: {
      ...
      resultAvailable: '结果已在私有 COS，点击后本地读取预览',
    ...
    guide: {
      ...
      step3: '3. 提交后任务会先排队，明细会展示已提交的 Prompt；图片预览默认不加载，点击后由浏览器从私有 COS 流式查找该图并缓存压缩缩略图。',
      step4: '4. 完成后可以下载 ZIP，ZIP 由浏览器本地生成，图片不经过 Sub2 日本服务器；部分失败时可以只重试失败项。',
    ...
    messages: {
      ...
      archiveClientFailed: '浏览器无法安全读取或解析私有 COS 结果。请检查 COS 跨域配置、短时签名或结果文件完整性。',
