              ...
              <table class="w-full">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-gray-900 dark:text-white">{{ usageTotal(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
              ...
              <table class="w-full">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-gray-900 dark:text-white">{{ usageTotal(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
const ringItems = computed<RingItem[]>(() => {
  ...
  if (data.mode === 'quota_limited') {
    if (data.quota) {
      ...
      items.push({ title: t('keyUsage.totalQuota'), pct, amount: `${quotaMoney(data.quota.used)} / ${quotaMoney(data.quota.limit)}`, iconType: 'dollar' })
    }
    if (data.rate_limits) {
      ...
      for (const rl of data.rate_limits) {
        ...
        items.push({
          ...
          pct,
          amount: `${quotaMoney(rl.used)} / ${quotaMoney(rl.limit)}`,
    ...
    if (data.subscription) {
      ...
      for (const l of limits) {
        if (l.limit != null && l.limit > 0) {
          ...
          items.push({ title: l.label, pct, amount: `${quotaMoney(l.usage)} / ${quotaMoney(l.limit)}`, iconType: 'calendar' })
        }
      }
    }
    if (!data.subscription && data.balance != null) {
      items.push({ title: t('keyUsage.walletBalance'), pct: 0, amount: quotaMoney(data.balance), isBalance: true, iconType: 'dollar' })
    }
  }
  ...
  if (data.mode === 'quota_limited') {
    if (data.quota) {
      ...
      rows.push({
        ...
        label: t('keyUsage.remainingQuota'), value: quotaMoney(data.quota.remaining), valueClass: remainColor,
      })
    }
    if (data.expires_at) {
    ...
    if (data.rate_limits) {
      ...
      for (const rl of data.rate_limits) {
        ...
        let valueStr = `${quotaMoney(rl.used)} / ${quotaMoney(rl.limit)}`
    ...
    if (data.subscription) {
      ...
      if (sub.daily_limit_usd > 0) {
        ...
        rows.push({
          iconBg: 'bg-primary-500/10', iconColor: 'text-primary-500', iconSvg: ICON_DOLLAR,
          label: `${t('keyUsage.usedQuota')} (${locale.value === 'zh' ? '日' : 'D'})`, value: `${quotaMoney(sub.daily_usage_usd)} / ${quotaMoney(sub.daily_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.weekly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${locale.value === 'zh' ? '周' : 'W'})`, value: `${quotaMoney(sub.weekly_usage_usd)} / ${quotaMoney(sub.weekly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.monthly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${locale.value === 'zh' ? '月' : 'M'})`, value: `${quotaMoney(sub.monthly_usage_usd)} / ${quotaMoney(sub.monthly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.expires_at) {
        rows.push({
    ...
    rows.push({
      iconBg: 'bg-emerald-500/10', iconColor: 'text-emerald-500', iconSvg: ICON_SHIELD,
      label: t('keyUsage.remainingQuota'), value: data.remaining != null ? quotaMoney(data.remaining) : '-', valueClass: remainColor,
    })
  }
...
const usageStatCells = computed<StatCell[]>(() => {
  ...
  return [
    ...
    { label: t('keyUsage.todayCost'), value: usageTotal(today.actual_cost) },
    ...
    { label: t('keyUsage.totalCost'), value: usageTotal(total.actual_cost) },
...
function usageTotal(value: number | null | undefined): string {
  return resultData.value?.unit === 'CNY' ? '—' : quotaMoney(value)
}

function quotaMoney(value: number | null | undefined): string {
  if (value == null || value < 0) return '-'
  return (resultData.value?.unit === 'CNY' ? '¥' : '$') + Number(value).toFixed(2)
}
