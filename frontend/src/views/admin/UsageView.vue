<template>
  <AppLayout>
    <div class="space-y-6">
      <UsageStatsCards :hide-mixed-currency-totals="appStore.cachedPublicSettings?.pricing_currency?.settlement_currency === 'CNY'" :stats="usageStats" />
      <!-- Charts Section -->
      <div class="space-y-4">
        <div class="card p-4">
...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      'Currency', t('usage.rate'), t('usage.accountMultiplier'), t('usage.original'), t('usage.userBilled'), t('usage.accountBilled'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.currency || 'USD', log.rate_multiplier?.toPrecision(4) || '1.00', (log.account_rate_multiplier ?? 1).toPrecision(4),
