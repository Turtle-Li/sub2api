  ...
  return mount(SettingsView, {
    global: {
      stubs: {
        ...
        BackupSettings: true,
        PricingCurrencySettingsCard: true,
      },
    },
  });
}
...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("does not expose TT Switch settings in the service admin", async () => {
    const wrapper = mountView();

    await flushPromises();

    expect(wrapper.find('[data-testid="desktop-control-plane-url-input"]').exists()).toBe(false);
    expect(wrapper.text()).not.toContain("TT Switch 合作推广");
  });
  ...
  it("keeps recharge editing out of settings and preserves the stored catalogue", async () => {
    getSettings.mockResolvedValueOnce({ ...baseSettingsResponse, payment_recharge_options: [] });
    const wrapper = mountView();
    await flushPromises();
    await openPaymentTab(wrapper);
    expect(wrapper.findAll("textarea").some(el => el.attributes("placeholder")?.startsWith('[{"amount":20'))).toBe(false);
    await wrapper.find("form").trigger("submit.prevent");
    await flushPromises();
    expect(updateSettings).toHaveBeenCalledTimes(1);
