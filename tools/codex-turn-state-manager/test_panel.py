...
class PanelTests(support.AccountsAndRetryTests):
    ...
    def test_remote_management_bind_rejected_before_socket(self):
        manager, *_ = self.make()
        for bind in ('0.0.0.0', '100.64.0.1', 'localhost'):
            manager.config['panel'] = dict(enabled=True, bind=bind, read_only=False)
            with mock.patch.object(mod, 'ThreadingHTTPServer') as server:
                with self.assertRaises(ValueError): mod.start_panel(manager)
                server.assert_not_called()

    def test_proxy_usage_migrates_secrets_and_preserves_cooldown(self):
        manager, *_ = self.make()
        proxy = 'http://private-user:private-password@example.test:80'
        manager._proxy_state_file.write_text(json.dumps({proxy: {'last_used': 123, 'last_state_len':292}}))
        data = manager._load_proxy_usage()
        self.assertEqual(data[manager._proxy_identity(proxy)]['last_used'],123)
        self.assertNotIn('private-password', manager._proxy_state_file.read_text())
        self.assertEqual(manager._proxy_state_file.stat().st_mode & 0o777,0o600)
        self.assertEqual(manager.state_dir.stat().st_mode & 0o777,0o700)

    def test_private_readonly_bind_allowed(self):
        manager, *_ = self.make()
        manager.config['panel'] = dict(enabled=True, bind='100.64.0.1', read_only=True)
        with mock.patch.object(mod, 'ThreadingHTTPServer') as server, mock.patch.object(mod.threading, 'Thread'):
            mod.start_panel(manager)
            self.assertEqual(server.call_args.args[0], ('100.64.0.1',8787))
